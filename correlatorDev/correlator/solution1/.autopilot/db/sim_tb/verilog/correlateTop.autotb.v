...
`define AESL_DEPTH_phaseClass_V 1
...
`define AUTOTB_TVIN_phaseClass_V  "./c.correlateTop.autotvin_phaseClass_V.dat"
...
`define AUTOTB_TVIN_phaseClass_V_out_wrapc  "./rtl.correlateTop.autotvin_phaseClass_V.dat"
...
parameter LENGTH_phaseClass_V = 1;
...
// The signal of port phaseClass_V
reg [3: 0] AESL_REG_phaseClass_V = 0;
assign phaseClass_V = AESL_REG_phaseClass_V;
initial begin : read_file_process_phaseClass_V
    integer fp;
    integer err;
    integer ret;
    integer proc_rand;
    ...
    transaction_idx = 0;
    wait(AESL_reset === 1);
    fp = $fopen(`AUTOTB_TVIN_phaseClass_V,"r");
    if(fp == 0) begin       // Failed to open file
        $display("Failed to open file \"%s\"!", `AUTOTB_TVIN_phaseClass_V);
        $display("ERROR: Simulation using HLS TB failed.");
        $finish;
    end
    read_token(fp, token);
    ...
    while (token != "[[[/runtime]]]") begin
        ...
        if(token != "[[/transaction]]") begin
            ret = $sscanf(token, "0x%x", AESL_REG_phaseClass_V);
              if (ret != 1) begin
                  $display("Failed to parse token!");
                $display("ERROR: Simulation using HLS TB failed.");
                  $finish;
...
reg end_phaseClass_V;
reg [31:0] size_phaseClass_V;
reg [31:0] size_phaseClass_V_backup;
