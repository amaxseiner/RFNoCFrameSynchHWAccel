...
void correlateTopSynch(hls::stream<rfnoc_axis> i_data, hls::stream<rfnoc_axis> o_data){
...
switch(currentState) {
	...
	case ST_LOAD: // whenever there is valid input data, shift it in
		...
		cor.shiftPhaseClassSynch(newValiDec,newValqDec,phaseClass);
		out = cor.correlatorSynch(phaseClass);
		//corHelperI = 0;
		//o_data->data = out.V;
...
void correlate::shiftPhaseClassSynch(cor_t newValuei,cor_t newValueq,ap_uint<4> phaseClass){
	switch(phaseClass){
	case 0:
